...
export default function App() {
  ...
  const exportZip = async () => {
    ...
    try {
      ...
      const zip = new JSZip()
      await showExportStage('Serializing bottom die', 'Writing binary STL triangles', 34)
      const maleStart = performance.now()
      zip.file('embossing-die-bottom.stl', geometryToBinaryStl(meshes.male.geometry, 'bottom'))
      timings.push({ phase: 'Serialize bottom STL', 'time (ms)': Number((performance.now() - maleStart).toFixed(2)) })
      await showExportStage('Serializing top die', 'Writing binary STL triangles', 56)
      const femaleStart = performance.now()
      zip.file('embossing-die-top.stl', geometryToBinaryStl(meshes.female.geometry, 'top'))
      timings.push({ phase: 'Serialize top STL', 'time (ms)': Number((performance.now() - femaleStart).toFixed(2)) })
